...
- `typos_config_builder/phrases.py` enforces the shared phrase corrections
  ...
  tracked-file selection for it (the Git listing, the policy-file and
  escape checks, and fail-closed reading) and is the only module that shells
  out to Git.
...
- Focused unit and command-boundary tests are kept in `tests/`, one module
  ...
  `test_policy.py`, `test_patterns.py`, `test_phrases.py`, `test_gate.py`,
  `test_gate_typos.py` for the Typos invocation contracts, `test_cli.py`),
